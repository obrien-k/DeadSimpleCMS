...
- Build failures are translated to human terms, with a one-click revert that commits the previous file content back. *(Translation landed in #9; the original example — "a stray quote in front matter" — turned out to be **unreachable**: the CST writer above cannot emit broken YAML, so the reachable red build is **body Liquid**, not front matter. Revert is still pending. See below.)*
...
2. **Content comfort:** drafts flow, ~~image upload + insertion~~ (landed in #14), ~~front-matter inference~~ (landed in #13), conflict handling, one-click revert *(build-failure translation landed in #9; revert still pending)*.
