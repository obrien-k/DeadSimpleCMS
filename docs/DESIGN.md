...
**`index.html` is the trust root; `bundle.js` is the only replaceable part.** `index.html` carries the `owner/repo` config line and the update-signing public key, and is written *only* by the installer. Self-update rewrites `bundle.js` alone, so the trust root is immutable for the life of the install.

**`/admin/` hard-refuses to run outside a secure context.** A page served over plain HTTP can be MITM'd and handed a token-stealing script, which makes every other defence here theatre — the API calls being HTTPS is irrelevant once the attacker is running *in the page*. `crypto.subtle` also does not exist outside a secure context, so update-signature verification would be unavailable. The setup wizard must verify **Enforce HTTPS** is on and block until it is: GitHub cannot enforce HTTPS on a custom domain until its certificate provisions, so "built, but plain HTTP" is a real state a real user hits while waiting.
...
- **Update integrity: a TOFU signature chain, not a hash.** Fetching a bundle and its hash from the same repo is circular — a compromised source serves a matching hash, so it proves nothing. Instead `index.html` carries a public key written at install time, and every update must be signed by the matching private key, verified via `crypto.subtle` before the commit. Cost is ~100 bytes; WebCrypto is native, so no library ships. Key loss means no further updates for existing installs (recoverable by reinstall); key rotation requires a reinstall, by design.
...
| Update | Self-commit + TOFU signature | The app already has `contents: write`; the bundle is fetched unauthenticated from the public project repo (`api.github.com` sends CORS `*`), so no CDN is needed. A hash check would be circular — same source serves bundle and hash — so `index.html` roots a signature chain instead, verified by native WebCrypto (~100 bytes) |
...
| Transport | Hard-refuse plain HTTP | A PAT on an MITM-able page makes every other defence theatre, and `crypto.subtle` needs a secure context. Observed live: Pages defaults `https_enforced: false` on a custom domain until the cert provisions |
