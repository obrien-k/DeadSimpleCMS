...
- **Listing** uses the **GraphQL API** in two phases, and never reads posts one by one (`prototype/post-listing/`, July 2026, measured against a 102-post site):
  - *Phase 1 — the only content call in steady state.* One query returns `entries { name oid }` for the resolved posts and drafts directories via aliases. Cost **1**. **It is no longer the app's only call**: resolving *where Jekyll reads from* (#17, below) costs a `GET /pages` + `GET /repos` pair first, so a load is **2 round trips**, or 3 on a `collections_dir` site. The steady-state listing itself is unchanged; the trip that used to be free was buying a wrong answer.
  - *Phase 2 — only on cache misses.* Blobs addressed **directly by oid**, one alias each: `b0: object(oid: "…") { ... on Blob { text } }`. Cost **1**; verified to 102 aliases in one query (`nodeCount` 0 — aliased oid lookups don't count against the node limit), so a cold start is one query at any realistic post count.
...
#### Where Jekyll actually looks (#17)
...
**Measured against Jekyll 4.4.1 and the live API (July 2026), not reasoned:**
...
| `_underscore/_posts/`, `.hidden/_posts/`, `excluded/_posts/`, `node_modules/_posts/` | ❌ — the walk prunes `_`, `.`, and `exclude` (whose defaults include `node_modules`) |
...
**Still open:** `**/_posts` recursion. #17 resolved the root only, so `postsDirs`/`draftsDirs` hold exactly one entry — which the table above proves is *still wrong* for a site with `blog/_posts/`. They are **arrays** so that fix lands inside `src/layout/` without moving a caller. The bounded walk (Jekyll's own `_`/`.`/`exclude` fence) is also what page discovery needs — same walk, same fence.
- Extra front-matter fields are **inferred by sampling the repo's existing posts** — no schema file, ever. A site whose posts all carry `image.path` gets that field in the form. *(Inference is phase 2. Note the form itself already adds keys to posts that lack them — see round-trip safety below — so key insertion is a phase-1 problem regardless.)*
- **The form's field order is `title → date → description → tags → categories → image`.** It is not cosmetic: it is the rule for where a new key gets inserted.
...
- **The post list has no upper bound.** `Tree.entries` is not a Relay connection: no pagination exists, so the phase-1 listing response is whatever the posts directory holds. Measured at ~104 B/post (name + oid), a 200-post site is ~7 kB gzip and a 2,000-post site ~70 kB — acceptable, but it is a response that grows forever with no mechanism to bound it. Revisit if a site ever gets absurd. **The `**/_posts` recursion (#17) makes this worse before it makes it better**: enumerating posts across every directory cannot be a fixed set of aliases, and the likely replacement — Trees `?recursive=1` — returns the whole repo's path list and carries its own `truncated` cliff (~100k entries / 7 MB). Jekyll's own prune rules (`_`, `.`, `exclude`) are the only fence, and they are a real one.
