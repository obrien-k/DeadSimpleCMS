...
- **Listing** uses the **GraphQL API** in two phases, and never reads posts one by one (`prototype/post-listing/`, July 2026, measured against a 102-post site):
  - *Phase 1 — the only call in steady state.* One query returns `entries { name oid }` for `_posts/` and `_drafts/` via two aliases. Cost **1**.
...
- **The post list has no upper bound.** `Tree.entries` is not a Relay connection: no pagination exists, so the phase-1 listing response is whatever `_posts/` holds. Measured at ~104 B/post (name + oid), a 200-post site is ~7 kB gzip and a 2,000-post site ~70 kB — acceptable, but it is a response that grows forever with no mechanism to bound it. Revisit if a site ever gets absurd.
